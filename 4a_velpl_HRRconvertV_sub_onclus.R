...
### INPUT and OUTPUT
#Input
# wdir <-  'C:\\Research\\Scale_sub_aveVel_0422\\Data\\HRR_velpl\\32_smva'
# wdir <- 'D:\\Data\\Trt_vel_ave_ss\\10_smva'
...
# fpattern = c("\\*plane*p1.csv$","\\*plane*p2.csv$","\\*plane*p3.csv$","\\*plane*p4.csv$","\\*plane*p5.csv$")

# fpattern = c("\\*plane*p6.csv$","\\*plane*p7.csv$","\\*plane*p8.csv$","\\*plane*p9.csv$","\\*plane*p10.csv$")

# fpattern = c("\\*plane*p9.csv$")
...
flist <- list.files(pattern = "plane_ss_fps", recursive = TRUE)
...
for (i in 1:length(flist)){
  ...
  for (j in 1: dim(hrrflow)[2]){
    ...
    flow_gg = hrrflow[2:dim(hrrflow)[1],j]
  ...
  # write to csv
  # outcsv <- paste(paste(wdir,outname,sep='\\'),'csv',sep='.')
  outcsv <- paste(outname,'csv',sep='.')
