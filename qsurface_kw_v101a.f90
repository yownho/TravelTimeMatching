...
!	Set qup for the next space step
        	qup=qe
        	!if(j.eq.10) write(*,*) q_pl_s(j,ndx)
        	
            !-----velocity on plane
!            if (y_pl_s(j,k1) .eq. 0) then
!                vel_pl_s(j,k1) = 0
!            else
!                vel_pl_s(j,k1) = q_pl_s(j,k1)/y_pl_s(j,k1)
!            endif
            !if(j.eq.310 .AND. k1.eq.10) write(*,*) j, vel_pl_s(j,k1),vel_pl_s_max(j,k1)
            ! Output velocity and depth all the time for model units
